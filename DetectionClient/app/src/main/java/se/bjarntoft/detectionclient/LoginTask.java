...
public class LoginTask extends AsyncTask<Object, Void, Boolean> {
    ...
    private LoginFragment parentFragment;

    @Override
    protected Boolean doInBackground(Object... params) {
        // Extraherar indata.
        parentActivity = (MainActivity)params[0];
        parentFragment = (LoginFragment)params[1];
        String request = (String)params[2];

        try {
            URL url = new URL(QuickstartPreferences.HOST + "login.php");
            ...
            if(urlConnection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                try {
                    ...
                    if(jsonArray.length() > 0) {
                        ...
                        sharedPreferences.edit().putString(QuickstartPreferences.USER_ID, id).apply();
                        sharedPreferences.edit().putString(QuickstartPreferences.USER_PASSWORD, password).apply();
                        sharedPreferences.edit().putBoolean(QuickstartPreferences.USER_LOGGED_IN, true).apply();
